...
const createPeer = async (peerId: number): Promise<RTCPeerConnection> => {
  ...
  pc.onicecandidate = (ev) => {
    if (ev.candidate) {
      try { ws.value?.send(JSON.stringify({ type: 'rtc:candidate', from: currentUserId, to: peerId, candidate: ev.candidate })) } catch {}
    }
  }
  pc.ontrack = (ev) => {
  ...
  pc.onnegotiationneeded = async () => {
    try {
      if (currentUserId < peerId) {
        ...
        try { ws.value?.send(JSON.stringify({ type: 'rtc:offer', from: currentUserId, to: peerId, sdp: offer })) } catch {}
      }
    } catch {}
  }
        ...
        ;(async () => {
          ...
          try {
            ...
            if (pc.signalingState === 'have-remote-offer') {
              ...
              try { ws.value?.send(JSON.stringify({ type: 'rtc:answer', from: currentUserId, to: from, sdp: answer })) } catch {}
            }
          } catch {}
        })()
      }
...
const send = () => {
  ...
  if (!text) return
  const msg = { type: 'message', id: crypto.randomUUID(), userId: currentUserId, name: currentUserName, avatar: currentUserAvatar, text, timestamp: Date.now() }
  inputText.value = ''
...
<template>
  ...
  >
    ...
    <header class="chat-header">
      ...
      <div class="peers">
        <div class="peer" v-for="u in users" :key="u.id" :title="u.name">{{ u.avatar }}</div>
      </div>
    </header>
...
.grabber {
  ...
  box-shadow: inset 0 0 0 1px #ffffff, 0 0 0 3px rgba(15,23,42,0.04), -6px 0 12px rgba(15,23,42,0.06);
}
